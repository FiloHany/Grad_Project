import { LayoutDashboard, PlusCircle, History, LogOut, User, Loader2 } from 'lucide-react'
...
export default function AppSidebar({ activeView, onNavigate, jobCount, isAnalyzing, doctor }) {
  ...
  return (
    <aside
      ...
      style={{ background: 'linear-gradient(180deg, #1e1b4b 0%, #0f0f23 40%, #080816 100%)' }}
    >
      {/* ── Logo ─────────────────────────────────────────────────────────── */}
      <div className="flex-shrink-0 px-5 py-5 border-b border-white/10">
        ...
        >
          ...
          <div className="leading-tight text-left">
            ...
            <div className="text-[10px] text-indigo-300 font-medium tracking-[0.15em] uppercase mt-0.5 opacity-50">
              Cardiac AI
            </div>
          </div>
        </button>
      ...
      <nav className="flex-1 overflow-y-auto px-3 pt-4 pb-2">
        <p className="text-[10px] font-bold tracking-[1.6px] text-white/20 uppercase px-3 pb-3">
          Menu
        </p>
            ...
            return (
              <li key={key}>
                ...
                >
                  {isLive
                    ...
                    : (
                      <Icon className={[
                        ...
                        isActive ? 'text-indigo-300' : 'text-white/30 group-hover:text-white/70',
                      ].join(' ')} />
                    )
                  }
                  ...
                  {isLive && (
                    <span
                      className="text-[9px] font-bold text-white px-2 py-0.5 rounded-full tracking-wide"
